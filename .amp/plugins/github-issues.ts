...
3. If it is a bug report, load the \`is\` skill with the skill tool and run its workflow on this issue. Treat its result as analysis, not authorization to edit.
...
5. If the oracle decides it should not be fixed, stop without changing code or GitHub state. If it should be fixed, check whether the issue is already fixed or has an active fix branch or pull request. Stop rather than duplicate existing work.
