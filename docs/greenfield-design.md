...
- **Policy loader**: the whole layering — defaults, user policy.json with section-wise salvage, project policy.json with merge + weakening lines, both rule.json (scope drops on malformation), live rulebooks (v1/v2 schema, name match, vendored-only), env with raise-only level, capability provenance; plus `readRetentionDays` sharing the salvage path. Landed 2026-09-08: the salvage normalizer is the single runtime validator. `salvageUserPolicy` returns the policy it left in force plus one drop per rejected field or section in the loader's own plain wording, `readPolicyFile` renders those drops as the file's diagnostics, and `validate.ts` — the hand-written mirror that reproduced the schema's sentences and its ordering — is deleted. The schema library runs only on the diagnostic surfaces (`doctor`, `policy check`, the GUI's read/preview/repair/write in `store-gui.ts`, `diff.ts`, the legacy-config writer), so nothing the hook path imports pulls it in. `rule.json` and rulebook validation stay hand-written and all-or-nothing, in `rules-config.ts` and `rulebook.ts`. Never throws for content; I/O error → source missing/malformed (degraded); a genuine bug throws → pipeline denies at config-load. → PolicyResolution. Prohibited: parser, analyzer, audit, adapters, network. If removed: every surface hand-rolls layering.
...
- **Destructive analyzer**: status handling; path-set walk (cwd knowledge, env tracking, heredoc files, function inlining, connector semantics, isolation, substitutions first); per-simple-command fixed order (assignment strip, wrapper peel, strict dynamic checks, transparent wrappers, eval/trap/shell wrappers/source/stdin/awk/interpreters/busybox/device/head rule sets/unknown-head suffix search/custom rules); worktree relaxation; raw-text backstop; rule filter; budgets; trace emission. Landed 2026-09-08: that per-command order is the one dispatcher. A child a producer synthesizes — `xargs`, `parallel`, a `find -exec` body, the unknown-head suffix scan — enters it as command words carrying a `ChildProvenance`: which producer built it, the directories and environment it runs in, whether a transparent wrapper opened it, and the producer's own flags and matches for the input it cannot see. Every point where such a child differs from the command as written reads that provenance and nothing else does, so custom-rule matching, built-in filtering, trace recording and budget accounting each happen in exactly one place; the separate child and embedded doors are gone. Wrapper peeling shares one algorithm (`wrapper-prelude.ts`) but is still driven from two sites: the dispatcher peels the command as written, and `normalizeChildCommands` peels a synthesized child (busybox, transparent wrappers, `env -S`) with its own `wrapperPeelIterations` counter before the child enters the dispatcher. Exposes its **path-tracking walk** as a function used by protections and secret stages — one walker, three consumers (superseded by §8.1: two walks by design). Deps: model, oracle, catalogs, budget, one sync `git config --get submodule.recurse` spawn (sanitized env, timeout) reachable only from worktree verification. Prohibited: policy loader (receives resolved capabilities), audit, adapters, network.
...
## 4.12 Extension
Open: new host = `hosts/<id>/` (hook adapter or entry + installer + detector + managed-command constant) + one entry in a plain array; Claude-shaped hosts = five-line family override. New destructive rule = one catalog record + one predicate in the head rule set (or a new head rule set wired into the dispatch table, which is the single per-command path, so a child synthesized by `xargs`, `parallel`, a `find -exec` body or the unknown-head suffix scan reaches it without a second wiring) + corpus rows at standard and strict (+ one raw-text pattern if vocabulary-visible). New secret pattern = catalog record + corpus. New rulebook source = data. New capability (rare) = four known sites documented, not abstracted. Closed: third-party code rules; new intents; network on decision path; alternative storage; resident process; parser fidelity for residual families (review rule); host descriptor DSL; GUI as independent app.
...
1. **No universal walker.** The guards need a resolved cwd for identity tests; the analyzer needs cwd knowledge that becomes unknown after any real change (`cd .. && rm -rf build` must deny). One walker serving both needs consumer flags. Build two walks by design: a linear guard walk with `cd` and simple-assignment tracking shared by the policy-file, policy-apply, git-metadata, and secret guards, and a control-flow walk for destructive analysis. This closes the reproduced gap where `cd ~ && cat .ssh/config` is allowed while `cat ~/.ssh/config` is denied, and keeps the defense in depth of protections that do not depend on the analyzer's walk. Landed 2026-09-08: the guard walk also carries the shell's directory scope. The projection emits `scope` entries around a nested program that runs in its own shell — a subshell group, `$( )`, backticks, a process substitution — and the walk saves its state (cwd, variables, previous directory) on enter and restores it on exit, so a `cd` inside one never reaches the parent, while a brace group and a function body share the parent's state because the shell runs them there; `cd -` swaps with the remembered previous directory, and `pushd`/`popd` stay untracked. A nested shell's words still join the segment around them, so each guard sees the same token bag as before. Landed 2026-09-08: the flat projection is gone with it. `readGuardSyntax` reads the parsed tree once and says whether the command is scannable; `walkGuardSyntax` replays what it read through a three-callback visitor — word, segment, redirection — and the policy-file, policy-apply, git-metadata and secret guards all drive that one walk with the directory scope described above. The word rules moved over unchanged because the tree does not carry them: `CommandWord.text` keeps `$HOME` verbatim, has no glob flag and does not split a quoted path out of an interpreter call, so the inertness pass, the substitution placeholder, the glob drop, the assignment fallbacks, the data-sink heredoc masking and the 256-expansion function inlining live in the walk. A redirection whose target the shell reads as an operand of the segment around it (`<<`, `<<<`, `>|`) is the one place the two old machines disagreed, so the visitor says which it wants. The analyzer keeps its own control-flow walk by design (§4.12).
...
3. **Lean hook entry behind the pinned path.** Keep `dist/bin/cc-safety-net.js`; the bin resolves the `hook` verb before importing the GUI, installers, and doctor through one dynamic import of a second chunk. Revised 2026-09-10: the bin is a CommonJS loader over a self-contained `dist/bin/hook.js` bundle (its directory carries a `package.json` marking it CommonJS), so the hook skips the ES module loader; the dynamic import targets the ESM `dist/cli.js` entry, and the loader enables Node's compile cache under the user's CC Safety Net home. `node:crypto` left the hook's static closure: the audit id and policy temp-file suffix come from `Math.random`. Take the schema validator off the hot path by having the salvage normalizer report dropped sections; run schema diagnostics only on diagnostic surfaces. Landed 2026-09-08 as written: the normalizer reports its drops and owns runtime acceptance, and the hand-written mirror that had been keeping the schema off the hook path is deleted (§4.2, Policy loader).
...
5. **Inject git facts through the env seam and add a spawn timeout.** Extend the environment seam with `gitMetadata(cwd)` and `worktreeFacts(cwd)`; the `git config --get submodule.recurse` spawn takes a timeout and a failure means no relaxation. Do not build a twelve-duty path oracle. Landed 2026-09-08: an `Environment` owns the memo behind those two facts, so ownership is per unit of work rather than per process — the policy GUI server takes a `() => Environment` factory and calls it once at the top of each request, so a `.git` marker created or removed under a long-lived server changes the next answer; the hook keeps one environment per process.
