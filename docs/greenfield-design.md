...
3. **Lean hook entry behind the pinned path.** Keep `dist/bin/cc-safety-net.js`; the bin resolves the `hook` verb before importing the GUI, installers, and doctor through one dynamic import of a second chunk. Take the schema validator off the hot path by having the salvage normalizer report dropped sections; run schema diagnostics only on diagnostic surfaces. Landed 2026-09-08 as written: the normalizer reports its drops and owns runtime acceptance, and the hand-written mirror that had been keeping the schema off the hook path is deleted (§4.2, Policy loader).
