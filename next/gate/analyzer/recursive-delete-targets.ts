...
import { isTrustedTempPath, isTrustedTempRootPath } from '@next/core/paths/tmpdir';
import { expandAllowPathHome, getAllowPathHomeConflictError } from '@next/core/policy/allow-paths';
import type { CommandWord } from '@next/core/shell/model';
import { expandPosixLiteralBraceWord } from '@next/core/shell/posix';
import type { EnvironmentContext, PathResolver, ProtectedGitMetadata } from '@next/gate/analysis';
import { isProtectedGitDeleteTarget } from '@next/gate/guards/git-metadata-protection';
...
export function classifyRecursiveDeleteTarget(
  ...
  if (anchoredCwd) {
    if (
      !options.skipHomeCwd &&
      isCwdHomeForRmPolicy(anchoredCwd, ctx.environment.home, ctx.environment.paths, ctx.budget)
    ) {
...
function isCanonicalHomeTarget(
  ...
  try {
    ...
    const resolved = normalizePathForComparison(
      resolveExistingPath(base, ctx.environment.paths, ctx.budget),
    );
    if (resolved === parse(resolved).root) return true;
    return (
      resolved ===
      normalizePathForComparison(
        resolveExistingPath(ctx.environment.home, ctx.environment.paths, ctx.budget),
      )
    );
  } catch {
    return false;
...
function isAllowedPathTarget(
  ...
  try {
    const canonical = normalizePathForComparison(
      resolveExistingPath(resolved, ctx.environment.paths, ctx.budget),
    );
