...
import { getActivitySummary } from '@next/cli/doctor/activity';
import { checkForUpdates } from '@next/cli/doctor/updates';
import { type RunInstallCommandOptions, runInstallCommand } from '@next/cli/install/index';
...
} from '@next/core/policy/diff';
import { mergeProjectPolicy } from '@next/core/policy/merge';
import { getProjectPolicyPath, type RulesPolicyOptions } from '@next/core/policy/paths';
...
import {
  ...
  describeConfigState,
  loadPolicySnapshot,
} from '@next/core/policy/snapshot';
import {
  createPolicyPreview,
  DEFAULT_GUI_POLICY,
  normalizeGuiPolicy,
...
} from '@next/core/policy/store';
import { getUserPolicyDiagnostics } from '@next/core/policy/validate';
import { DESTRUCTIVE_COMMAND_RULE_METADATA } from '@next/core/rules/destructive';
import { SECRET_PROTECTION_RULE_METADATA } from '@next/core/rules/secret';
import { type ExplainResult, explainCommand } from '@next/gate/explain';
import { getIntegrationDisplayName, installIntegrationMetadata } from '@next/hosts/catalog';
import { detectAllHooks } from '@next/hosts/detect/index';
import type { SystemInfo, UpdateInfo } from '@next/hosts/doctor-types';
import {
  INSTALL_TARGETS,
  type InstallAction,
  type InstallTarget,
} from '@next/hosts/install/targets';
import { getPackageVersion, getSystemInfo, type VersionFetcher } from '@next/hosts/system-info';
