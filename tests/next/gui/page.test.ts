...
import { guiDocument } from '@next/gui/assets';
 ...
 * the order the three helper modules land in. Everything else, the session token included, must be
 * the shipped document byte for byte.
 */

const FRONTEND = join(import.meta.dir, '..', '..', '..', 'next', 'gui', 'frontend');
...
describe('the served GUI page', () => {
  test('is the shipped page once the bundler labels and module order are folded out', () => {
    ...
    const shipped = normalizePage(pages.shipped, TOKEN);
  ...
  test('a token that closes the data tag parses back to itself on both sides', () => {
    ...
    expect(dataPayload(pages.ported)).toStrictEqual(dataPayload(pages.shipped));
  });
