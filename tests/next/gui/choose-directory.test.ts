...
} from '@next/gui/choose-directory';
import {
  isDirectoryPickerAvailable as shippedAvailable,
  chooseDirectory as shippedChoose,
 ...
 * The port is byte-identical to the shipped module, so every row runs both and compares the answer
 * and the argv the fake dialog recorded — a fake `zenity`, `kdialog` or missing `osascript` on a
 * `PATH` of our own, so no test opens a window.
 */
...
describe('whether a folder dialog can be opened', () => {
  ...
  test('agrees with the shipped answer on every platform and PATH shape', () => {
    ...
    for (const [platform, env, available] of rows) {
      expect(portedAvailable(platform, env)).toBe(shippedAvailable(platform, env));
...
describe('opening the folder dialog', () => {
  ...
  const runSide = async (
    side: string,
    row: (typeof DIALOG_ROWS)[number],
    choose: typeof shippedChoose,
  ) => {
    const root = createTempRoot(`gui-picker-${side}-`);
    const fake = createFakeBin(root, row.script(root));
    if (row.picked) mkdirSync(join(root, 'picked'), { recursive: true });
    const result = await choose(row.platform, { ...fake.env, DISPLAY: ':0' });
  ...
  test.each(
    ...
    const ported = await runSide('ported', row, portedChoose);
    expect(ported).toStrictEqual(await runSide('shipped', row, shippedChoose));
    expect(ported).toStrictEqual({ result: row.result, argv: row.argv });
  });
});
