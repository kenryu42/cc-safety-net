 ...
 * and the two install routes, driven against both servers over twin homes. Every hook that would
 * spawn `gh`, a dialog, an installer or a browser is injected per side and records what it was
...
describe('the GUI activity feed over HTTP', () => {
  // `os.homedir()` resolves once at process start, so `withProcessEnv` cannot move it and the
  // shipped feed would report the developer's home where the ported one reports the seeded one.
  // The spy hands the shipped side the home its own requests already run under, which is what a
  // real `cc-safety-net gui` reads.
...
describe('the GUI rulebook listing', () => {
  ...
  test('reports the platform dialog it could not open rather than opening one', async () => {
    ...
    // Which message depends on the platform; that both sides answered with the same one is the row.
...
describe('the GUI star, integrations and install endpoints', () => {
  ...
  test('runs an install and an uninstall for the target the body names, and nothing else', async () => {
    const calls: Record<string, [string, string][]> = { shipped: [], ported: [] };
    const row = await runGuiRow({
      seed: {},
      options: (_side, label) => ({
        runIntegration: async (action, target) => {
          calls[label]?.push([action, target]);
    ...
    // The refused bodies never reached the installer on either side.
    expect(calls.ported).toStrictEqual(calls.shipped);
    expect(calls.shipped).toStrictEqual([
      ['install', 'cursor'],
      ['uninstall', 'codex'],
    ]);
  });
