...
import {
  fetchStarContext as shippedStarContext,
  starRepo as shippedStarRepo,
  userHasStarredRepo as shippedUserHasStarred,
} from '@/gui';
...
const SIDES: readonly (readonly ['shipped' | 'ported', StarHelpers])[] = [
  ['shipped', { userHasStarredRepo: shippedUserHasStarred, starRepo: shippedStarRepo }],
  ['ported', { userHasStarredRepo: portedUserHasStarred, starRepo: portedStarRepo }],
];

/**
 * The two implementations over their own scripted `gh`, one side after the other: `runGhCommand`
 * spawns with the process environment, so the fake's script and log are handed to it that way, and
 * an overlap would let one side spawn against the other's script. The command itself is an absolute
 * path into the fake bin, so `PATH` is never touched.
 */
const bothSides = async <T>(
  script: readonly FakeScriptEntry[],
  run: (side: {
    label: 'shipped' | 'ported';
    helpers: StarHelpers;
    command: string;
    root: string;
  }) => Promise<T>,
  ...
  const observed: { result: T; calls: (string | undefined)[] }[] = [];
  for (const [label, helpers] of SIDES) {
    const root = createTempRoot(`gui-star-${label}-`);
    ...
    const result = await withProcessEnv(
      ...
      async () => run({ label, helpers, command: join(fake.binDir, 'gh'), root }),
    );
    // The working directory each call ran in is the suite's own, so only the argv is comparable.
    observed.push({ result, calls: fake.readLog().map((line) => line.split('\t')[0]) });
  }
  expect(observed[1]).toStrictEqual(observed[0] as (typeof observed)[number]);
  return observed[0] as { result: T; calls: string[] };
};
  ...
  const contextOver = async (fetchRepo: typeof fetch) => {
    const row = await bothSides(
      ...
      async (side) => {
        ...
        const options = { command: side.command, logsDir: seedLogs(home), fetchRepo };
        return side.label === 'shipped'
          ? shippedStarContext(options)
          : portedStarContext(environmentFor(home, values), options);
      },
