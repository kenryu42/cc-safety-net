 ...
 * and anything one session hit twice on the same command signature. Both halves are verbatim ports
 * — the signature comes from the shared display helper, the rule from the page script — so the
 * ported page must carry the shipped bytes, and the pair is then run over fresh entries.
 */
