 ...
 * which directory that is, and which rulebook names are already taken. The block is a verbatim
 * port, so the ported page must carry the shipped bytes, and it is then run over its own state.
 */
