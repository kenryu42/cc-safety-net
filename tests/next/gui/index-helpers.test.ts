...
import {
  fetchHealth as shippedHealth,
  fetchIntegrations as shippedIntegrations,
  runIntegration as shippedRunIntegration,
} from '@/gui';
 ...
 * table, the overview health strip and the installer the Integrations tab drives. The shipped
 * helpers take the home as a parameter and the ported ones read it off the `Environment`, so every
 * row runs both over twin seeded homes with a stubbed version fetcher — nothing here spawns a host
 * CLI, and no probe reaches npm or the network.
 */
...
const integrationsOver = async (seed: TreeSpec) => {
  const outcome = expectSameSides(
    await differential({
      seed,
      shipped: (home) => shippedIntegrations({ fetcher: mockVersionFetcher, homeDir: home }),
