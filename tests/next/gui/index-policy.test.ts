...
/**
 * The guard, the page and the user-policy endpoints, driven against both servers over twin homes.
 * Every row is the differential itself; the assertions below pin what the shipped side answered, so
 * a row that starts agreeing on the wrong thing still fails.
 */

const USER_POLICY_FILE = 'home/.cc-safety-net/policy.json';
const PROJECT_POLICY_FILE = 'project/.cc-safety-net/policy.json';
const USER_POLICY_PATH = `<root>/${USER_POLICY_FILE}`;
...
describe('the policy GUI server', () => {
  ...
  test('reports a project policy in force beside the user file', async () => {
    ...
    expect(weakened.projectPolicy?.path).toBe(`<root>/${PROJECT_POLICY_FILE}`);
