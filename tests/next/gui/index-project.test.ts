...
import { runPolicyCommand as runPortedPolicy } from '@next/cli/policy/index';
import { DEFAULT_GUI_POLICY } from '@next/core/policy/store';
import { runPolicyCommand as runShippedPolicy } from '@/cli/policy';
 ...
 * that follows it, and the containment the write goes through. Every row drives both servers over
 * twin homes, so a body, a status or a file that moved is the failure; the pins behind each row
 * stop the two sides agreeing on nothing.
 */
...
describe('the GUI project draft', () => {
  ...
  test('follows the directory the picker chose and binds each write to its revision', async () => {
    ...
    expect(after).toMatchObject({ dir: '<root>/picked', revision: 1 });
    ...
    expect(row.responses[5]).toMatchObject({
      status: 200,
      body: { path: '<root>/picked/.cc-safety-net/policy.json', errors: [] },
    });
  ...
  test('binds an apply to the revision it read at entry, not the one the pick left behind', async () => {
    ...
    expect(row.responses[0]).toMatchObject({
      status: 200,
      body: { path: `<root>/${PROJECT_POLICY_FILE}`, errors: [] },
    });
  ...
  test('leaves the draft where it was when the dialog cancels or fails', async () => {
    ...
    for (const row of [cancelled, failed]) {
      expect(row.responses[1]?.body).toMatchObject({ dir: '<root>/project', revision: 0 });
    }
  });
  ...
  test('shows the rows `policy check` prints for the same proposal', async () => {
    ...
    const checked = [];
    for (const label of ['shipped', 'ported'] as const) {
      const root = createTempRoot(`gui-check-${label}-`);
      ...
      const run = () =>
        label === 'shipped'
          ? withProcessEnv(values, () =>
              captureConsole(() =>
                runShippedPolicy(['check', file], { cwd: join(root, 'project') }),
              ),
            )
          : captureConsole(() =>
              runPortedPolicy(environmentFor(home, values), ['check', file], {
    ...
    expect(checked[1]).toStrictEqual(checked[0]);
    expect(checked[0]?.returned).toBe(0);
    expect(diff.rows.length).toBeGreaterThan(0);
    for (const row of diff.rows) {
      expect(checked[0]?.log).toContain(
