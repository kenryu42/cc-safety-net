 ...
 * machine, and the issue URL it builds under GitHub's length limit. The block is a verbatim port,
 * so the ported page must carry the shipped bytes, and it is then run on its own to prove what
 * those bytes do.
 */
