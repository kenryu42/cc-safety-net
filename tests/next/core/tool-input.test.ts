...
describe('next/core/tool-input against src/parser/tool-input', () => {
  ...
  test('rejects unsafe and oversized shapes with the same limit error', () => {
    ...
    const runners = {
      command: [next.getCommandFromToolInput, src.getCommandFromToolInput],
      paths: [
        (input: unknown) => next.extractPathLikeToolValues(input, PATH_LIKE_KEYS),
        (input: unknown) => src.extractPathLikeToolValues(input, PATH_LIKE_KEYS),
      ],
      targets: [next.extractPatchTargetsFromToolInput, src.extractPatchTargetsFromToolInput],
    };

    for (const row of rejected) {
      for (const kind of row.throwsIn) {
        for (const run of runners[kind]) {
          expect(() => run(row.input)).toThrow('tool input traversal limit exceeded');
        }
      }
    }
  });
