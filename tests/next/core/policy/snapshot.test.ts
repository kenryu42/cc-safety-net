...
import type { RulesPolicyOptions } from '@next/core/policy/paths';
import { loadPolicySnapshot as loadPortedSnapshot } from '@next/core/policy/snapshot';
import type { PolicySnapshot } from '@next/core/policy/types';
 ...
 * port has to agree with the shipped loader on every row of `docs/config-recovery.md` — the
 * whole snapshot, not just its state. Each row builds its own tree under one temp root, runs
 * both loaders over the same files, compares the snapshots, and checks the documented
 * fallback on the ported one. The tree is captured before and after: loading writes nothing.
 */
...
describe('the policy loader port reproduces every configuration recovery row', () => {
  ...
  test.each(
    ...
    try {
      ...
      const expected = withEnv(env, () => loadPolicySnapshot(options));
