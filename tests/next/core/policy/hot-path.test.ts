...
const NEXT = join(import.meta.dir, '..', '..', '..', '..', 'next');
...
// `createRequire` is how the shipped schema module reaches its validator, so the walk
...
function moduleFileFor(specifier: string, importer: string): string | null {
  ...
  const base = specifier.startsWith('@next/')
    ? join(NEXT, specifier.slice('@next/'.length))
