...
import { bindPolicyFilesystemScope } from '@next/core/io/safe-read';
import {
  getRulesConfigRuntimeErrorsForConfig,
  getUnknownOverrideErrorsForConfig,
} from '@next/core/policy/scope-policy';
import { bindPolicyFilesystemScope as shippedBindPolicyFilesystemScope } from '@/rules/policy/filesystem';
import {
  getRulesConfigRuntimeErrorsForConfig as shippedRuntimeErrors,
  getUnknownOverrideErrorsForConfig as shippedUnknownOverrideErrors,
 ...
 * to know its rule ids. Each row resolves both on the shipped module and on the ported one over
 * the same tree built twice.
 */
...
/** The same scope under one root per implementation, so neither can see the other's. */
function roots() {
  const shipped = createTempRoot('scope-policy-shipped-');
  const ported = createTempRoot('scope-policy-ported-');
  writeTree(shipped, TREE);
  writeTree(ported, TREE);
  return { shipped, ported };
}
...
function reportsFor(scope: string, bound: boolean) {
  ...
  const shipped = {
    runtime: shippedRuntimeErrors(
      configPath(root.shipped, scope),
      bound ? shippedBindPolicyFilesystemScope(root.shipped, 'project policy') : undefined,
    ),
    overrides: shippedUnknownOverrideErrors(
      configPath(root.shipped, scope),
      bound ? shippedBindPolicyFilesystemScope(root.shipped, 'project policy') : undefined,
    ),
  };
