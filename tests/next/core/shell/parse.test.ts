...
import type { CommandParserLimits, ShellKind } from '@next/core/shell/model';
import { DEFAULT_COMMAND_PARSER_LIMITS, parseCommand } from '@next/core/shell/parse';
import {
...
describe('next/core/shell/parse against src/parser/command', () => {
  ...
  test('parses the corpus, the fixed table and the seeded fuzz identically in every dialect', () => {
    ...
    for (const pair of pairs) {
      expect({ source: pair.source, dialect: pair.dialect, program: pair.next }).toStrictEqual({
        source: pair.source,
  ...
  test('with small custom limits, both implementations agree and report limited', () => {
    for (const row of overCap) {
      for (const dialect of row.dialects) {
        ...
        expect({ source: row.source, dialect, program }).toStrictEqual({
          source: row.source,
          dialect,
