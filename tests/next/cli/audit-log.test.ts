 ...
 * directory it finds — so the comparison folds it to one token.
 */
...
async function runLogs(args: readonly string[], row: Omit<CliRow, 'args'> = {}) {
  const result = await runCliDifferential({ args: ['logs', ...args], ...row });
  return expectSameCli({
    shipped: foldProjectDir(result.shipped),
    ported: foldProjectDir(result.ported),
  });
}

/** The same rows against the seeded tree; the clock is read once so both sides get one fixture. */
