...
import type { DoctorReport } from '@next/hosts/doctor-types';
import {
  type CliRow,
  type CliSide,
  expectSameCli,
...
describe('doctor rendered', () => {
  test('a fresh home renders the self-test and the single error finding', async () => {
    const shipped = expectSameCli(
      await runCliDifferential({ args: ['doctor', '--skip-update-check'] }),
    );
    expect(shipped.exitCode).toBe(1);
    expect(shipped.stdout).toContain('Guard Engine Verification');
    expect(shipped.stdout).toContain('3/3 passed');
    expect(shipped.stdout).toContain('1 finding: 1 error.');
  }, 120_000);
  ...
  test('a seeded audit tree renders its activity header', async () => {
    ...
    expect(shipped.stdout).toContain('Recent Activity · last 7 days (3 blocked / 1 sessions)');
    expect(shipped.stdout).toContain('git reset --hard');
  }, 120_000);
  ...
  test('an unknown option is refused before anything is inspected', async () => {
    const shipped = expectSameCli(await runCliDifferential({ args: ['doctor', '--nope'] }));
    expect(shipped.exitCode).toBe(1);
    expect(shipped.stdout).toBe('');
    expect(shipped.stderr).toBe('Unknown option for doctor: --nope\n');
  }, 60_000);
});
