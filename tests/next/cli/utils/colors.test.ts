...
} from '@next/cli/utils/colors';
import {
  colorizeToken as shippedColorizeToken,
  colors as shippedColors,
  generateDistinctColor as shippedGenerateDistinctColor,
  shouldUseColor as shippedShouldUseColor,
} from '@/cli/utils/colors';
import { withStdoutTTY } from '../../helpers/fake-tty';
import { withProcessEnv } from '../../helpers/temp-home';

/**
 * `shouldUseColor()` is read per call, so the whole surface is sampled twice under one fixed
...
function sample(
  colors: typeof shippedColors,
  generateDistinctColor: typeof shippedGenerateDistinctColor,
  colorizeToken: typeof shippedColorizeToken,
) {
  return {
    named: NAMES.map((name) => colors[name]('x')),
    distinct: Array.from({ length: 8 }, (_value, index) => generateDistinctColor(index, 3)),
    token: colorizeToken('tok', 2, 3),
  };
}

const shippedSample = () =>
  sample(shippedColors, shippedGenerateDistinctColor, shippedColorizeToken);
const portedSample = () => sample(portedColors, portedGenerateDistinctColor, portedColorizeToken);
  ...
  test('a TTY without NO_COLOR colors both implementations identically', () => {
    withStdoutTTY(true, () =>
      withProcessEnv({ NO_COLOR: undefined }, () => {
        expect(shippedShouldUseColor()).toBe(true);
        expect(portedShouldUseColor()).toBe(true);
        expect(portedSample()).toEqual(shippedSample());
    ...
    test(`${terminal.label} on both implementations`, () => {
      withStdoutTTY(terminal.isTTY, () =>
        withProcessEnv({ NO_COLOR: terminal.noColor }, () => {
          expect(shippedShouldUseColor()).toBe(false);
