...
import type { ExplainResult } from '@next/gate/explain';
import type { TraceStep } from '@next/gate/trace';
import {
  type CliOutcome,
  type CliRow,
  expectSameCli,
 ...
 * fixed command are compared byte for byte between the two bins and then pinned to a literal
 * golden written from the shipped side. The pin next to each row names what the row exists to
...
/** The rendering both bins produced, against the literal file. `CC_SAFETY_NET_UPDATE_GOLDENS=1`
 *  rewrites it from the shipped side instead of comparing. */
function pinRendering(name: string, result: { shipped: CliOutcome; ported: CliOutcome }): void {
  const path = goldenFile(name);
  if (process.env.CC_SAFETY_NET_UPDATE_GOLDENS === '1') {
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, result.shipped.stdout);
    return;
  }
  const golden = readFileSync(path, 'utf-8');
  expect(result.shipped.stdout).toBe(golden);
  expect(result.ported.stdout).toBe(golden);
}
...
/**
 * The inputs design §8.4 changed on purpose. Each is run through both bins, asserted to still
 * differ, and the ported verdict is pinned so the change stays the decided one rather than
 * whatever the port happens to produce.
 */
const KNOWN_GAPS = [
  {
    name: 'a partial program is analyzed as raw text, not as the command it resembles',
    // `src` explain analyzed the partial program and reported `git.reset-hard`; the `src` hook
    // and the ported pipeline both answer with the raw-text rule. §8.4 drops the divergence.
  ...
  },
  {
    ...
    // `src` recorded `fallback-scan` and `custom-rules-check`.
    command: "echo 'unterminated",
    ported: { result: 'allowed', ruleId: undefined },
  },
  {
    name: 'a PowerShell command is matched in its own dialect',
    // `src` explain routed the pre-analysis protections as posix, so `$env:HOME` read as the
    // variable `env` followed by literal text; the pipeline parses the real dialect.
...
describe('explain diverges from the shipped CLI only where the design says it must', () => {
  for (const gap of KNOWN_GAPS) {
    test(gap.name, async () => {
      ...
      expect(result.ported.stdout).not.toBe(result.shipped.stdout);
      const document = JSON.parse(result.ported.stdout) as ExplainResult;
  ...
  // words under either dialect, so the change is invisible here and they are compared instead.
  ...
  // In strict mode the partial program is unparseable to both, and both say so the same way.
