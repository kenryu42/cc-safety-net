...
import * as portedFormat from '@next/cli/rule/format';
import * as shippedFormat from '@/cli/rule/format';
...
describe('the rule report both implementations print', () => {
  for (const row of rows) {
    test(row.name, async () => {
      const shipped = await captureConsole(() => row.print(shippedFormat));
      const ported = await captureConsole(() => row.print(portedFormat));
      expect(ported).toStrictEqual(shipped);
      expect(shipped.log).toEqual([...row.log]);
      expect(shipped.error).toEqual([...row.error]);
    });
  }
});
