...
} from '@next/cli/rule/sync-migrate';
import {
  findRuleV2Leftovers as findShipped,
  runRuleSyncMigration as runShippedMigration,
...
async function runMigration(spec: TreeSpec, global: boolean) {
  return runManagerDifferential(spec, (side, environment) =>
    captureConsole(() => {
      const options = global ? { cwd: side.project, global: true } : { cwd: side.project };
      return environment ? runPortedMigration(environment, options) : runShippedMigration(options);
    }),
  );
}
