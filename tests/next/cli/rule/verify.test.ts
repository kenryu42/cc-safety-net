...
import { runRulesVerify as portedRulesVerify } from '@next/cli/rule/verify';
import { runRulesVerify as shippedRulesVerify } from '@/cli/rule/verify';
...
function verifyBothWays(spec: TreeSpec) {
  const shipped = runVerify('shipped', spec, (context) =>
    withProcessEnv(context.env, () => shippedRulesVerify({ cwd: context.cwd })),
  );
...
describe('rule verify', () => {
  ...
  test('a valid user config gains its schema and lists its sources', () => {
    ...
    expect(outcome.report).toContain('✓ User config: <root>/home/.cc-safety-net/rules/rule.json');
  ...
  test('a rulebook whose fixture fails is reported by index', () => {
    ...
    expect(outcome.code).toBe(1);
    expect(outcome.report).toContain('✗ GitHub source rules: <root>/project/.cc-safety-net/rules');
    expect(outcome.report).toContain(
  ...
  test('a legacy project config is reported against the project scope', () => {
    ...
    expect(outcome.code).toBe(1);
    expect(outcome.report).toContain('✗ Legacy project config: <root>/project/.safety-net.json');
    expect(outcome.report).toContain(
