...
import { runRuleCommand as portedRuleCommand } from '@next/cli/rule/index';
import { getUpdateNotice as portedGetUpdateNotice } from '@next/cli/rule/update-notice';
import * as portedSystemInfo from '@next/hosts/system-info';
import { runRuleCommand as shippedRuleCommand } from '@/cli/rule';
import { getUpdateNotice as shippedGetUpdateNotice } from '@/cli/rule/update-notice';
import * as shippedSystemInfo from '@/integrations/system-info';
...
import { environmentFor, removeTempRoots, withProcessEnv } from '../../helpers/temp-home';

/**
 * The notice `rule doc` appends. Every row fixes `now`, pins the running version through the
 * package-version export both implementations read, and answers the registry from a recorder, so
 * what is compared is the decision itself: whether the poll happened, what the cache kept and
...
describe('the update notice both implementations decide on', () => {
  for (const row of rows) {
    test(row.name, async () => {
      const spies = [
        spyOn(shippedSystemInfo, 'getPackageVersion').mockReturnValue(RUNNING),
      ...
      try {
        const agreed = await runManagerDifferential(
          ...
          async (side, environment) => {
            ...
            // A row that moves a variable needs both sides to read the moved value, so the
            ...
            const notice = environment
              ? await portedGetUpdateNotice(environmentFor(side.home, values), NOW)
              : await withProcessEnv(values, () => shippedGetUpdateNotice(NOW));
            return { notice, urls };
          },
        );
    ...
    test(
      ...
      async () => {
        const spies = [
          spyOn(shippedSystemInfo, 'getPackageVersion').mockReturnValue(RUNNING),
          spyOn(portedSystemInfo, 'getPackageVersion').mockReturnValue(RUNNING),
          // `rule doc` asks for the notice without a `now`, so without a fixed clock the two
          // sides would stamp their caches milliseconds apart and differ over that alone.
          spyOn(Date, 'now').mockReturnValue(NOW),
        ];
        try {
          const agreed = await runManagerDifferential({}, async (side, environment) => {
            globalThis.fetch = ((input: Parameters<typeof fetch>[0]) => {
            ...
            return captureConsole(() =>
              environment
                ...
                : withProcessEnv(values, () => shippedRuleCommand(['doc'])),
            );
          });
