...
} from '@next/cli/startup/banner';
import {
  awaitWithSpinner as shippedAwaitWithSpinner,
  resolveAfterOptionalBanner as shippedResolveAfterOptionalBanner,
 ...
 * by a timer, and both implementations see the identical release order.
 */
...
async function driveSpinner(
  awaitWithSpinner: typeof shippedAwaitWithSpinner,
  options: { frames: number; isTTY?: boolean; fail?: boolean },
) {
...
async function driveBanner(
  resolveAfterOptionalBanner: typeof shippedResolveAfterOptionalBanner,
  showBanner: boolean,
) {
  const calls: string[] = [];
  const value = await resolveAfterOptionalBanner(
    showBanner,
    () => {
      calls.push('start');
      return {
...
describe('cli/startup/banner', () => {
  test('work that beats the delay draws nothing on either implementation', async () => {
    const ported = await driveSpinner(portedAwaitWithSpinner, { frames: 0 });
    expect(ported).toEqual(await driveSpinner(shippedAwaitWithSpinner, { frames: 0 }));
  ...
  test('three ticks draw the same three frames on both implementations', async () => {
    const ported = await driveSpinner(portedAwaitWithSpinner, { frames: 3 });
    expect(ported).toEqual(await driveSpinner(shippedAwaitWithSpinner, { frames: 3 }));
    expect(ported).toEqual({
      chunks: [
        HIDE_CURSOR,
        spinnerFrame(0),
  ...
  test('a non-TTY sink passes the value through on both implementations', async () => {
    const ported = await driveSpinner(portedAwaitWithSpinner, { frames: 0, isTTY: false });
    expect(ported).toEqual(
      await driveSpinner(shippedAwaitWithSpinner, { frames: 0, isTTY: false }),
    );
    expect(ported).toEqual({
      chunks: [],
      outcome: { kind: 'returned', value: 'ready-value' },
  ...
  test('the banner runs between starting and finishing the work on both implementations', async () => {
    const withBanner = await driveBanner(portedResolveAfterOptionalBanner, true);
    expect(withBanner).toEqual(await driveBanner(shippedResolveAfterOptionalBanner, true));
    expect(withBanner).toEqual({ calls: ['start', 'banner', 'finish'], value: 'done' });

    const withoutBanner = await driveBanner(portedResolveAfterOptionalBanner, false);
    expect(withoutBanner).toEqual(await driveBanner(shippedResolveAfterOptionalBanner, false));
