 ...
 * for one seeded scope; `apply` is driven twice in-process because a real terminal is the one
 * thing the process harness cannot hand it.
 */
...
async function applyBothWays(extra: readonly string[], answer: (input: PassThrough) => void) {
  const shipped = await driveApply('shipped', extra, answer, (context) =>
    withProcessEnv(context.env, () =>
      shippedPolicyCommand(context.args, {
        cwd: context.cwd,
        input: context.input,
