 ...
 * calls, the files written and removed, and every line the command prints. Each row runs the
 * shipped and the ported flow over identically seeded homes with the same fake CLIs on `PATH`,
 * so a divergence in a message, a command, an exit code or a byte fails the row.
 */
