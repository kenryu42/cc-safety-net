...
describe('statusline', () => {
  test('a disabled plugin is the whole answer', async () => {
    const shipped = await runStatusline({
      args: ['statusline', '-cc'],
      stdin: HOST_PAYLOAD,
    });
    expect(shipped.exitCode).toBe(0);
    expect(shipped.stdout).toBe('🛡️ CC Safety Net ❌\n');
  }, 60_000);

  test('a non-JSON payload is echoed in front of the line', async () => {
    const shipped = await runStatusline({ args: ['statusline', '-cc'], stdin: 'hello' });
    expect(shipped.stdout).toBe('hello | 🛡️ CC Safety Net ❌\n');
  }, 60_000);
  ...
  for (const [name, row, expected] of [
    ...
    test(`an enabled plugin at ${name}`, async () => {
      ...
      expect(shipped.stdout).toBe(expected);
    }, 60_000);
  }
  ...
  test('a malformed user policy adds the degraded glyph', async () => {
    ...
    expect(shipped.stdout).toBe('🛡️ CC Safety Net ✅⚠️\n');
  }, 60_000);
  ...
  test('the flag is required', async () => {
    const shipped = await runStatusline({ args: ['statusline'], stdin: HOST_PAYLOAD });
    ...
    expect(shipped.stderr).toContain('cc-safety-net statusline');
  }, 60_000);
