...
} from '@next/hosts/openclaw/plugin';
import {
  createOpenClawBeforeToolCallHandler as shippedHandler,
  registerOpenClawPlugin as shippedRegister,
...
/**
 * The OpenClaw `before_tool_call` hook driven through a fake plugin API: the same event, context
 * and workspace go to the shipped and the ported handler, and the returned decision, the stderr
 * lines and the audit tree must agree. The last case is the port's own contract — a host context
 * that throws denies in OpenClaw's own form instead of escaping the handler.
 */
...
function runSide(row: Row, side: Side) {
  const create = side === 'shipped' ? shippedHandler : portedHandler;
  const handler = create(createFakeApi(fixture, row.workspace).api, {
    guardDependencies: row.breaks ? { analyzeCommand: failingAnalyzer } : undefined,
  });
  return captureInProcessCall(fixture, side, row.env ?? {}, () =>
...
test('the debug line names the failing OpenClaw hook', async () => {
  ...
  const debugged = await runSide(row as Row, 'ported');
...
test('a host context that throws blocks in OpenClaw form instead of escaping the handler', async () => {
  ...
  const call = (side: Side) => {
    const handler = side === 'shipped' ? shippedHandler : portedHandler;
    return captureInProcessCall(fixture, side, {}, () =>
      handler(createFakeApi(fixture, undefined).api)(exec({ command: 'git status' }), hostile),
    );
  };

  expectFallbackDeny(await call('ported'), await call('shipped'), {
...
test('the allow row records the command OpenClaw was about to run', async () => {
  const allowed = await runSide(ROWS[1] as Row, 'ported');
