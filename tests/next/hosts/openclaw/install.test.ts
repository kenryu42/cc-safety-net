...
import {
  assertOpenClawPluginDirIsOurs as shippedAssertOurs,
  getOpenClawConfigPath as shippedConfigPath,
  findOpenClawArtifactDir as shippedFindArtifactDir,
  getOpenClawInstallCommands as shippedInstallCommands,
  getOpenClawPluginDir as shippedPluginDir,
  resolveOpenClawArtifactDir as shippedResolveArtifactDir,
  verifyOpenClawPluginRuntime as shippedVerifyRuntime,
    ...
    expect(
      expectSameSides(
        await differential({
          ...
          env,
          shipped: (home) => ({
            config: shippedConfigPath(home),
            plugin: shippedPluginDir(home),
          }),
          ported: (environment) => ({
...
describe('guarding the extension directory before a --force command', () => {
  const guard = async (seed: TreeSpec) =>
    expectSameSides(
      await differential({
        seed,
        shipped: (home) => describeOutcome(() => shippedAssertOurs(home)),
...
describe('finding the packaged plugin directory', () => {
  test('agrees on the built directory the installed CLI ships with', () => {
    expect(findOpenClawArtifactDir()).toBe(shippedFindArtifactDir() as string);
    expect(findOpenClawArtifactDir()).toBe(join(REPO_ROOT, 'dist', 'openclaw', 'cc-safety-net'));
  });

  test.each([
    ['nothing at any candidate path', ['gone']],
    ['a regular file where the directory should be', ['file']],
    ['the first directory among the candidates', ['gone', 'packaged']],
  ])('resolves %s the same way', (_case, names) => {
    const root = createTempRoot('next-openclaw-artifact-');
    writeTree(root, { file: 'not a directory', 'packaged/index.js': 'built' });
    const candidates = names.map((name) => join(root, name));

    expect(findOpenClawArtifactDir(candidates)).toBe(shippedFindArtifactDir(candidates) as string);
    expect(describeOutcome(() => resolveOpenClawArtifactDir(candidates))).toEqual(
      describeOutcome(() => shippedResolveArtifactDir(candidates)),
    );
  });
  ...
  test('drives the same two commands over the packaged directory', () => {
    expect(getOpenClawInstallCommands('/packaged/dir')).toEqual(
      shippedInstallCommands('/packaged/dir'),
    );
    expect(getOpenClawInstallCommands('/packaged/dir')).toEqual([
...
describe('verifying that the installed plugin actually loads', () => {
  const verify = async (entry: Omit<FakeScriptEntry, 'command' | 'args'>) => {
    ...
    const ported = await runOne('ported', verifyOpenClawPluginRuntime);
    expect(ported).toEqual(await runOne('shipped', shippedVerifyRuntime));
    return ported;
  };
