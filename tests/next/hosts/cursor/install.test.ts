...
import { detect as shippedDetectCursor } from '@/integrations/cursor/detect';
import {
  installCursor as shippedInstallCursor,
  uninstallCursor as shippedUninstallCursor,
} from '@/integrations/cursor/install';
...
const { row, detection } = hostRunner({
  shipped: (home) => ({
    install: () => shippedInstallCursor(home),
    detect: () => shippedDetectCursor({ homeDir: home, cwd: home }),
    uninstall: () => shippedUninstallCursor(home),
  }),
  ported: (environment) => ({
