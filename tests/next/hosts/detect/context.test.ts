...
import {
  ...
  readRecord,
  readStateFile,
} from '@next/hosts/detect/context';
import {
  inspectManagedPluginDir as shippedInspectManagedPluginDir,
  lstatOrUndefined as shippedLstatOrUndefined,
  readRecord as shippedReadRecord,
  readStateFile as shippedReadStateFile,
...
describe('the shared detector context', () => {
  test('tells a missing state file apart from an unreadable one', () => {
    ...
    expect(read(readStateFile)).toEqual(read(shippedReadStateFile));
  ...
  test('probes a path without following the link that sits on it', () => {
    ...
    expect(probe(lstatOrUndefined)).toEqual(probe(shippedLstatOrUndefined));
  ...
  test('refuses a managed plugin path that is not a real directory', () => {
    ...
    expect(inspect(inspectManagedPluginDir)).toEqual(inspect(shippedInspectManagedPluginDir));
  ...
  test('reads a key only out of a real object', () => {
    ...
    expect(read(readRecord)).toEqual(read(shippedReadRecord));
