...
} from '@next/hosts/pi/tool-call';
import {
  createPiToolCallHandler as shippedHandler,
  registerToolCallEvent as shippedRegister,
} from '@/integrations/pi/tool-call';
...
/**
 * The Pi `tool_call` event driven through a fake extension context: the same event, the same
 * working directory and the same policy options reach the shipped and the ported handler, and the
 * returned decision, the stderr lines and the audit tree must agree. The last case is the port's
 * own contract — a context that throws blocks in Pi's form instead of escaping the handler.
 */
...
test('the block for a failing secret scan names no command', async () => {
  ...
  const blocked = await runSide(row, 'ported');
...
test('the debug line names the failing Pi event', async () => {
  ...
  const debugged = await runSide(row, 'ported');
...
test('an unusable context directory is the one the audit records', async () => {
  ...
  expect((await runSide(row, 'ported')).entries[0]?.entry).toMatchObject({
    decision: 'deny',
    agent: 'pi',
    cwd: fixture.file,
  });
...
test('registering the event claims tool_call on both sides', () => {
  ...
  expect(named(record(portedRegister))).toStrictEqual(named(record(shippedRegister)));
...
test('a context that throws blocks in Pi form instead of escaping the handler', async () => {
  ...
  const call = (side: Side) =>
    captureInProcessCall(fixture, side, {}, () =>
      (side === 'shipped' ? shippedHandler : portedHandler)({})(bash('git status'), hostile),
    );
