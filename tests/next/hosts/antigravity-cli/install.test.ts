...
import { detect as detectAntigravity } from '@next/hosts/antigravity-cli/detect';
import {
  installAntigravityCli,
  uninstallAntigravityCli,
} from '@next/hosts/antigravity-cli/install';
import { detect as shippedDetectAntigravity } from '@/integrations/antigravity-cli/detect';
import {
  installAntigravityCli as shippedInstallAntigravity,
  uninstallAntigravityCli as shippedUninstallAntigravity,
...
const { row, detection } = hostRunner({
  shipped: (home) => ({
    install: () => shippedInstallAntigravity(home),
    detect: () => shippedDetectAntigravity({ homeDir: home, cwd: home }),
    uninstall: () => shippedUninstallAntigravity(home),
  }),
  ported: (environment) => ({
