...
import {
  detect as detectHermes,
  isHermesAgentPluginEnabled,
} from '@next/hosts/hermes-agent/detect';
import { buildHermesAgentPluginFiles } from '@/integrations/hermes-agent/artifact';
import {
  detect as shippedDetectHermes,
  isHermesAgentPluginEnabled as shippedIsEnabled,
} from '@/integrations/hermes-agent/detect';
 ...
 * YAML parser, so every shape the shipped reader accepts is pinned here against the port.
 */
...
const detection = async (seed: TreeSpec) =>
  expectSameSides(
    await differential({
      seed,
      shipped: (home) => shippedDetectHermes({ homeDir: home, cwd: home }),
    ...
    expect(
      expectSameSides(
        await differential({
          ...
          shipped: (home) => shippedIsEnabled(home),
