...
import { detect as detectHermes } from '@next/hosts/hermes-agent/detect';
import {
  installHermesAgent,
  readOwnedHermesAgentFiles,
  uninstallHermesAgent,
} from '@next/hosts/hermes-agent/install';
import { buildHermesAgentPluginFiles } from '@/integrations/hermes-agent/artifact';
import { detect as shippedDetectHermes } from '@/integrations/hermes-agent/detect';
import {
  installHermesAgent as shippedInstallHermes,
  readOwnedHermesAgentFiles as shippedReadOwnedHermes,
  uninstallHermesAgent as shippedUninstallHermes,
...
const { row } = hostRunner({
  shipped: (home) => ({
    install: () => shippedInstallHermes(home),
    detect: () => shippedDetectHermes({ homeDir: home, cwd: home }),
    uninstall: () => shippedUninstallHermes(home),
  }),
  ported: (environment) => ({
...
describe('removing the Hermes Agent plugin', () => {
  const uninstallOnly = async (seed: TreeSpec) =>
    expectSameSides(
      await differential({
        seed,
        shipped: (home) => describeOutcome(() => shippedUninstallHermes(home)),
...
describe('reading the owned Hermes Agent files', () => {
  const owned = async (seed: TreeSpec) =>
    expectSameSides(
      await differential({
        seed,
        shipped: (home) => describeOutcome(() => shippedReadOwnedHermes(home)),
