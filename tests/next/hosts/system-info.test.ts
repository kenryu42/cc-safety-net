...
import { installIntegrationMetadata } from '@next/hosts/catalog';
import {
  defaultVersionFetcher,
  getPackageVersion,
  getSpawnCommand,
  getSystemInfo,
} from '@next/hosts/system-info';
import {
  defaultVersionFetcher as shippedDefaultVersionFetcher,
  getPackageVersion as shippedGetPackageVersion,
  getSpawnCommand as shippedGetSpawnCommand,
  getSystemInfo as shippedGetSystemInfo,
...
describe('the Windows-safe argv', () => {
  test('hands a shim to COMSPEC and spawns everything else directly', () => {
    ...
    expect(resolved(getSpawnCommand)).toEqual(resolved(shippedGetSpawnCommand));
...
describe('the default version probe', () => {
  test('reads a clean exit only, and strips whatever painted it', async () => {
    ...
    const [ported, shipped] = await withProcessEnv(bin.env, async () => [
      await probe(defaultVersionFetcher),
      await probe(shippedDefaultVersionFetcher),
    ]);
...
describe('the system report', () => {
  test('probes every host once and parses whatever each one printed', async () => {
    ...
    expect(ported).toEqual(await record(shippedGetSystemInfo));
  ...
  test('reports the build-time package version', () => {
    ...
    expect(getPackageVersion()).toBe(shippedGetPackageVersion());
  });
});
