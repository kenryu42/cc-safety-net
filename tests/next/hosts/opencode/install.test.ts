...
import {
  clearOpenCodeCache as shippedClearCache,
  getOpenCodeConfigDir as shippedConfigDir,
  uninstallOpenCode as shippedUninstall,
  verifyOpenCodePluginRuntime as shippedVerify,
    ...
    expect(
      expectSameSides(
        await differential({
          ...
          shipped: (home) => shippedConfigDir(home),
  ...
  test.each([
    ...
    const result = expectSameSides(
      await differential({
        seed,
        env,
        shipped: (home) => shippedClearCache(home),
...
describe('proving the cached plugin would load', () => {
  const verify = async (seed: TreeSpec) =>
    expectSameSides(
      await differential({
        seed,
        shipped: (home) => shippedVerify(home),
...
describe('taking the plugin back out of the config', () => {
  const uninstall = async (seed: TreeSpec) => {
    const result = expectSameSides(
      await differential({
        seed,
        shipped: (home) => shippedUninstall(home),
