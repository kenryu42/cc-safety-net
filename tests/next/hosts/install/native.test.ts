...
import * as nextNative from '@next/hosts/install/native';
import * as srcNative from '@/integrations/install/native';
...
/** Run the same call against both implementations, each with its own fake bin and call log. */
async function forBoth<T>(run: (native: NativeModule) => Promise<T>) {
  ...
  return {
    ported: await runOne('ported', nextNative),
    shipped: await runOne('shipped', srcNative),
  };
}
...
describe('running a host CLI', () => {
  test('merges both streams, or narrows the success value to stdout on request', async () => {
    const { ported, shipped } = await forBoth(async (native) => ({
  ...
  test('reports the exit status and everything the command printed', async () => {
    const { ported, shipped } = await forBoth((native) =>
  ...
  test('reports a command that is not on PATH as a spawn failure', async () => {
    ...
    // what both implementations owe is the same wording plus the command that could not run.
