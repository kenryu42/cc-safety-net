...
import { detect as detectKimi } from '@next/hosts/kimi-code/detect';
import { installKimiCode, uninstallKimiCode } from '@next/hosts/kimi-code/install';
import { detect as shippedDetectKimi } from '@/integrations/kimi-code/detect';
import {
  installKimiCode as shippedInstallKimi,
  uninstallKimiCode as shippedUninstallKimi,
} from '@/integrations/kimi-code/install';
...
const { row, detection } = hostRunner({
  shipped: (home) => ({
    install: () => shippedInstallKimi(home),
    detect: () => shippedDetectKimi({ homeDir: home, cwd: home }),
    uninstall: () => shippedUninstallKimi(home),
  }),
  ported: (environment) => ({
