...
import { AnalysisLimit, REASON_SAFETY_NET_FAILED_CLOSED } from '@next/core/budget';
import { createProcessEnvironment } from '@next/core/environment';
import { ToolInputLimitError as PortedToolInputLimitError } from '@next/core/tool-input';
import { evaluateRuntimeGuard as portedEvaluateRuntimeGuard } from '@next/hosts/runtime';
import { PathCanonicalizationLimitError } from '@/analyzer/path-canonicalization';
import { evaluateRuntimeGuard as shippedEvaluateRuntimeGuard } from '@/integrations/runtime';
 ...
 * through both implementations against one audit home and compares what came back — or what was
 * thrown — together with the line that reached the log. The two limit rows raise each side's own
 * limit class, because the port replaced three exception types with one `AnalysisLimit`, and the
 * classification they share is what the audit line has to keep.
 */
...
const BREACHES = {
  unexpected: {
    shipped: () => new Error('injected analyzer failure'),
    ported: () => new Error('injected analyzer failure'),
  },
  toolInput: {
    shipped: () => new ToolInputLimitError(),
    ported: () => new PortedToolInputLimitError(),
  },
  canonicalization: {
    shipped: () => new PathCanonicalizationLimitError(),
    ported: () => new AnalysisLimit('pathEnvironmentExpansion'),
  },
} as const;
...
for (const row of ROWS) {
  test(row.name, () => {
    ...
    const guard = (side: 'shipped' | 'ported') => ({
      auditAllowed: row.auditAllowed,
      dependencies: row.breach ? throwing(BREACHES[row.breach][side]) : {},
    });

    const shipped = withEnv({ CC_SAFETY_NET_AUDIT_HOME: auditHome }, () => {
      const outcome = outcomeOf(() =>
        shippedEvaluateRuntimeGuard(call, { guard: guard('shipped'), audit: AUDIT }),
      );
      const entries = readAuditEntries(auditHome);
      clearAuditLogs(auditHome);
      return { outcome, entries };
    });
    const ported = withEnv({ CC_SAFETY_NET_AUDIT_HOME: auditHome }, () => {
      const outcome = outcomeOf(() =>
        portedEvaluateRuntimeGuard(createProcessEnvironment(), call, {
          guard: guard('ported'),
          audit: AUDIT,
        }),
      );
    ...
    if (row.entry) expect(shipped.entries[0]?.entry).toMatchObject(row.entry);
  });
}
