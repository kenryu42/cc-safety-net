...
import { detect as detectCopilot } from '@next/hosts/copilot-cli/detect';
import type { HookDetection } from '@next/hosts/detect/context';
import { detect as shippedDetectCopilot } from '@/integrations/copilot-cli/detect';
...
const detection = (
  ...
  detectionRunner({
    shipped: (home) =>
      shippedDetectCopilot({ homeDir: home, cwd: join(home, 'repo'), copilotCliVersion }),
    ported: (environment) =>
      detectCopilot({
        environment,
