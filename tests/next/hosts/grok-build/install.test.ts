...
import { detect as detectGrok } from '@next/hosts/grok-build/detect';
import { installGrokBuild, uninstallGrokBuild } from '@next/hosts/grok-build/install';
import { detect as shippedDetectGrok } from '@/integrations/grok-build/detect';
import {
  installGrokBuild as shippedInstallGrok,
  uninstallGrokBuild as shippedUninstallGrok,
} from '@/integrations/grok-build/install';
...
const { row, detection } = hostRunner({
  shipped: (home) => ({
    install: () => shippedInstallGrok(home),
    detect: () => shippedDetectGrok({ homeDir: home, cwd: home }),
    uninstall: () => shippedUninstallGrok(home),
  }),
  ported: (environment) => ({
...
describe('the Grok Build hook config differential', () => {
  ...
  test('leaves content it cannot parse in place instead of deleting it', async () => {
    ...
    const removal = expectSameSides(
      await differential({
        seed: { [HOOKS]: seed },
        shipped: (home) => describeOutcome(() => shippedUninstallGrok(home)),
