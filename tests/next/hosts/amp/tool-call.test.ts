...
import { createAmpToolCallHandler as portedHandler } from '@next/hosts/amp/tool-call';
import { createAmpToolCallHandler as shippedHandler } from '@/integrations/amp/tool-call';
 ...
 * the fake resolves to the fixture project, and the shell command through Amp's own extractor, so
 * the shipped and the ported handler see the same call. The returned action, the stderr lines and
 * the audit tree must agree; the last case is the port's own contract — an API that throws rejects
 * in Amp's form instead of escaping the handler.
 */
...
test('the rejection for a failing secret scan names no command', async () => {
  ...
  const rejected = JSON.stringify((await runSide(row, 'ported')).returned);
...
test('an API that throws rejects in Amp form instead of escaping the handler', async () => {
  ...
  const call = (side: Side) =>
    captureInProcessCall(fixture, side, {}, () =>
      (side === 'shipped' ? shippedHandler : portedHandler)({})(shell('git status'), hostile),
    );
