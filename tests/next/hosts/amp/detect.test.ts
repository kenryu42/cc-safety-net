...
import { detect } from '@next/hosts/amp/detect';
import type { HookDetection } from '@next/hosts/detect/context';
import { detect as shippedDetect } from '@/integrations/amp/detect';
  ...
  test.each(OUTPUTS)('reports %s', async (_case, ampPluginListOutput, expected) => {
    expect(
      expectSameSides(
        await differential({
          seed: {},
          shipped: (home) => shippedDetect({ homeDir: home, cwd: home, ampPluginListOutput }),
          ported: (environment) =>
