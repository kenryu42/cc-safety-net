...
import { AMP_MANAGED_HEADER } from '@next/hosts/amp/artifact';
import {
  ...
  uninstallAmp,
} from '@next/hosts/amp/install';
import type { AmpRunner } from '@next/hosts/amp/run';
import type { InstallResult } from '@next/hosts/install/types';
import {
  ampArtifactCandidates as shippedCandidates,
  installAmp as shippedInstallAmp,
  getAmpPluginPath as shippedLocalPath,
  resolveAmpArtifactPath as shippedResolveArtifact,
  uninstallAmp as shippedUninstallAmp,
} from '@/integrations/amp/install';
      ...
      shipped: (home) =>
        drive((run) =>
          row.action === 'uninstall'
            ? shippedUninstallAmp(home, run)
            : shippedInstallAmp(home, join(artifactRoot, 'index.ts'), run),
        ),
      ported: (environment) =>
        drive((run) =>
          row.action === 'uninstall'
  ...
  test('names the same system-scope path on both sides', async () => {
    expect(
      expectSameSides(
        await differential({
          seed: {},
          shipped: (home) => shippedLocalPath(home),
...
describe('finding the packaged artifact', () => {
  test('resolves the same shipped dist path from either module', () => {
    ...
    expect(candidates.at(-1)).toBe(shippedCandidates().at(-1) as string);
    // Both modules sit three directories under the repository root, so the packaged artifact the
    // release stamps is the one an install without an explicit path publishes.
    expect(candidates.at(-1)).toBe(
      join(import.meta.dir, '..', '..', '..', '..', 'dist', 'amp', ENTRY),
    );
  });
  ...
  test.each([
    ...
    expect(resolveAmpArtifactPath(candidates)).toBe(shippedResolveArtifact(candidates));
  ...
  test('reports a missing packaged artifact the same way', () => {
    ...
    expect(describeOutcome(() => resolveAmpArtifactPath(candidates))).toEqual(
      describeOutcome(() => shippedResolveArtifact(candidates)),
    );
    expect(describeOutcome(() => resolveAmpArtifactPath(candidates))).toEqual({
      ok: false,
      error: {
