...
import {
  detect as detectClaudeCode,
  hasClaudeInstalledPlugin,
} from '@next/hosts/claude-code/detect';
import type { HookDetection } from '@next/hosts/detect/context';
import {
  detect as shippedDetectClaudeCode,
  hasClaudeInstalledPlugin as shippedHasInstalledPlugin,
} from '@/integrations/claude-code/detect';
...
const detection = detectionRunner({
  shipped: (home) => shippedDetectClaudeCode({ homeDir: home, cwd: home }),
...
describe('asking whether a specific plugin id is installed', () => {
  const legacyInstalled = async (seed: TreeSpec) =>
    expectSameSides(
      await differential({
        seed,
        shipped: (home) => shippedHasInstalledPlugin(home, LEGACY_ID),
