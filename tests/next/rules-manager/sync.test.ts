...
import * as portedLimits from '@next/rules-manager/resource-limits';
import * as portedSync from '@next/rules-manager/sync';
import type {
  AddRulebookSourceOptions,
  AddRulebookSourceResult,
  SyncRulesConfigOptions,
  SyncRulesConfigResult,
} from '@next/rules-manager/types';
import * as shippedLimits from '@/rules/policy/resource-limits';
import * as shippedSync from '@/rules/policy/sync';
...
/**
 * Everything the manager writes, refuses to write, or rolls back, driven twice: the shipped
 * implementation over `process.env` and the ported one over its `Environment`, against one scripted
 * GitHub on loopback. A row compares the result objects and both trees; the messages it pins come
 * from the shipped side, so a reworded diagnostic or a byte that moves in `rule.json` fails here
...
/**
 * The manager both implementations expose, with the ported side's `Environment` already bound.
 * Without it every row would spell each call twice and the two spellings could drift apart; the
 * type names only the members a row actually calls.
 */
type Manager = {
...
function manager(environment?: Environment): Manager {
  if (!environment) {
    return {
      operation: shippedLimits.createRuleSyncOperation,
      budget: shippedLimits.createRuleSyncResourceBudget,
      sync: shippedSync.syncRulesConfigWithOperation,
      plainSync: shippedSync.syncRulesConfig,
      syncWithHooks: shippedSync.syncRulesConfigWithHooks,
      add: shippedSync.addRulebookSourceWithOperation,
      plainAdd: shippedSync.addRulebookSource,
      addWithHooks: shippedSync.addRulebookSourceWithHooks,
      remove: shippedSync.removeRulebookSource,
      removeWithHooks: shippedSync.removeRulebookSourceWithHooks,
      map: shippedSync.mapRulebookSources,
    };
  }
  return {
...
/** The request log this side produced, emptied so the other side starts from nothing. Fanout
 *  order is not deterministic, so the two logs are compared as sets of calls. */
...
describe('adding rulebook sources', () => {
  test('adds a local source and reports what the guard would load', async () => {
    const { results, tree, sides } = await addRow(
    ...
    expectGateView(sides, 'project', results);
  });

  test('adds every rulebook a repository publishes and vendors the served bytes', async () => {
    const { results, tree, sides } = await runManagerDifferential(
      { [CONFIG]: rulesConfig([]) },
    ...
    expectGateView(sides, 'project', results.result);
  });
...
describe('updating rulebook sources', () => {
  ...
  test('an unchanged upstream reports no change and writes nothing', async () => {
    const { results, tree, sides } = await syncRow(vendoredCatalog(['a', 'b']), { refresh: true });
...
describe('removing rulebook sources', () => {
  ...
  test('removes an exact local spec', async () => {
    const { results, tree, sides } = await removeRow(LOCAL_SCOPE, 'local');
...
describe('the post-change runtime reload', () => {
  ...
  test('--check reports what the guard would refuse, not just what each source validates', async () => {
    const { results, sides } = await syncRow(withUnknownOverride, { check: true });
    expect(results).toStrictEqual({
      ok: false,
  ...
  test('an add leaves the reload to the update that follows it', async () => {
    ...
    expectGateView(sides, 'project', results.updated);
  });
});
...
describe('fault hooks', () => {
  ...
  // the scripted GitHub is the global `fetch` both implementations call.
  beforeEach(() => {
