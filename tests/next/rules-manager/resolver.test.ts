...
import * as portedResolver from '@next/rules-manager/resolver';
import * as portedLimits from '@next/rules-manager/resource-limits';
import * as shippedResolver from '@/rules/policy/resolver';
...
/**
 * The resolver is the only part of the manager that touches the network, so every row here drives
 * both implementations against the same scripted GitHub on loopback and compares what each one
 * produced, what it spent from its budget, and which requests it made. The caps are asserted from
...
/** Only the members a row calls, spelled with the arguments a row passes: the two filesystem
 *  scopes are module-private nominal types, so the shipped and ported namespaces are
 *  interchangeable only for calls that leave the scope to its default. */
type Side = {
...
const SIDES = [
  { ...shippedResolver, ...shippedLimits },
  { ...portedResolver, ...portedLimits },
] as [Side, Side];
...
/** The same call on both implementations, each over its own copy of the seeded scope, compared
 *  whole with the requests it made; the agreed observation comes back for the row to pin. */
