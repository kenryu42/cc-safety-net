...
import * as budgetModule from '@next/core/budget';
...
import { analyzeCommandWithProgram, analyzerCapBreach } from '@next/gate/analyzer';
import { evaluateGuard } from '@next/gate/pipeline';
 ...
 * assigns that kind instead of failing closed, and that the shipped gate reaches the same verdict.
 ...
 * counts on one Budget. Every `createBudget` call made inside `next/` is counted, so a guard or an
 * analyzer entry that quietly builds its own budget again shows up as a second call. The rows below
 * run the stages that used to create their own: the protection walk with a tracked cwd and
 * an assignment, a heredoc the analyzer tracks as a file, and the three derived `rm` children —
 * `find -exec`, an `xargs` pipeline and a `parallel` expansion.
 */
describe('one Budget per evaluation', () => {
  // Real metadata, so the git-metadata guard does its path work instead of returning at `!metadata`.
  const inRepository = {
    loadPolicySnapshot: () => snapshot,
    resolveGitMetadata: () => resolveProtectedGitMetadata([tree.repository]),
  };
