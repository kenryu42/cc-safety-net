...
import { analyzeCommandWithProgram as shippedAnalyzeCommand } from '@/analyzer';
...
import { outputFailedClosed as shippedOutputFailedClosed } from '@/integrations/hook/common';
 ...
 * cap a command can still reach. The port has to fail the same way as the shipped gate, with the
 * same words, and — where a breach escapes as an exception — the same audit classification.
 */
...
describe('a filesystem that throws instead of answering', () => {
  ...
  // The analyzer is the one seam both gates take their process state through, so it is the only
  // place a broken `realpath` can be injected into the shipped gate and the port alike.
  ...
  const shippedBreakingPaths: Partial<ShippedDependencies> = {
    analyzeCommand: (command, options, program, store) =>
...
describe('a host that truncated the tool input', () => {
  ...
  function truncatedDenial(
    report: typeof shippedOutputFailedClosed | typeof portedOutputFailedClosed,
    toolInput: unknown,
  ) {
...
describe('process state the two gates read differently', () => {
  ...
  test('a rule config replaced between open and read degrades both snapshots alike', () => {
    ...
    withEnv({ CC_SAFETY_NET_HOME: safetyNetHome, HOME: tree.home }, () => {
      ...
      writeFileSync(rulePath, original);
      const shipped = shippedVerdict(call, {});
      spy.mockRestore();
...
/** The shipped audit classification, by the classes `src/integrations/runtime.ts` tests for. */
function shippedAuditCode(cause: unknown) {
  if (cause instanceof PathCanonicalizationLimitError) return 'path-canonicalization-limit';
...
/** The port's classification: one exception carrying the kind the `LIMITS` table codes. */
...
describe('every cap a command can still reach', () => {
  for (const breach of CAP_BREACHES) {
    ...
    test(`${breach.name} is classified the same way for the audit`, () => {
      ...
      expect({
        shipped: shippedAuditCode(
          escapingCause(() => shippedEvaluateGuard(call, { dependencies })),
        ),
        ported: portedAuditCode(
          escapingCause(() => portedEvaluateGuard(call, { environment, dependencies })),
        ),
      }).toStrictEqual({ shipped: breach.audited, ported: breach.audited });
    });
  }
