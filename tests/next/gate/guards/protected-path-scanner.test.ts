...
import {
  expandTrackedShellVariables as shippedExpandTrackedShellVariables,
  extractMvOperandPaths as shippedExtractMvOperandPaths,
  findProtectedPathMutationInCommand as shippedFind,
  isAssignmentOnlySegment as shippedIsAssignmentOnlySegment,
  normalizeProtectedFileCandidate as shippedNormalizeFile,
  normalizeProtectedPathCandidate as shippedNormalizePath,
 ...
 * redirection targets reach the guard. A divergence here silently unprotects a path, so the
 * differential records every callback the walk makes, not only what it returns.
 */
...
/**
 * The three observing callbacks, shared by both walks: the shipped state type and the ported one
 * are the same shape, so only `normalizeCwd` — whose seam differs — is supplied per side.
 */
...
function walkWithShipped(
  ...
  const observations: Observation[] = [];
  const result = shippedFind(
    shippedProjectShellSyntax(source, shippedParseCommand(source, 'posix')),
    cwd,
    createPathCanonicalizationContext(environment),
    { ...observing(observations, stop), normalizeCwd: shippedNormalizePath },
  );
  return { result, observations };
}

/** Both walks over one source — value or thrown error — so a caller can compare them. */
function walkPair(
  source: string,
  cwd: string,
  stop: string | null,
): [Outcome<Walk>, Outcome<Walk>] {
  const environments = pairedEnvironments({ HOME: home, TMPDIR: join(root, 'tmp') }, home);
  return [
    describeOutcome(() => walkWithNext(source, cwd, environments.next, stop)),
    describeOutcome(() => walkWithShipped(source, cwd, environments.shipped, stop)),
  ];
}
  ...
  test('the fixed table moves the tracked cwd and returns targets', () => {
    const trackedCwds = new Set(
      CD_SOURCES.flatMap((source) =>
        completedWalk(walkPair(source, workspace, null)[0]).observations.flatMap(
    ...
    expect(
      completedWalk(
        walkPair(`cd ${join(root, 'link')} && rm -rf x`, workspace, null)[0],
      ).observations.some((observation) => observation.includes(`cwd=${join(root, 'policy')}`)),
    ).toBeTrue();
    expect(
      SEGMENT_SOURCES.map(
        (source) => completedWalk(walkPair(source, workspace, MARKER)[0]).result,
  ...
  test('a structural-limit projection throws on both sides, an incomplete one is malformed', () => {
    ...
    expect(() =>
      shippedFind(
        facts,
        workspace,
        createPathCanonicalizationContext(pairedEnvironments({}, home).shipped),
        { ...observing(observations, null), normalizeCwd: shippedNormalizePath },
      ),
    ).toThrow('Structural command analysis limit exceeded.');
...
test('the walk charges one shared budget', () => {
  const budget = createBudget();
  const environment = pairedEnvironments({ HOME: home }, home).next;
  const observations: Observation[] = [];
  findProtectedPathMutationInCommand(
