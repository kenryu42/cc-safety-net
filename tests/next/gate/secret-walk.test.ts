...
import { bashCall, createGateTree, shippedVerdict } from '../helpers/gate-differential';
 ...
 * resolves and, where it says so, that the shipped gate still allows the same command — that
 * difference is the point of the change, not an accident of it.
 *
...
const ROWS: readonly {
  ...
  /** The shipped gate allows this command; the walk is the whole reason the port denies it. */
...
for (const row of ROWS) {
  test(row.name, () => {
    ...
    if (row.closedByWalk === true) {
      expect(
        shippedVerdict(bashCall(row.command, tree.workspace), {
          loadPolicySnapshot: () => SNAPSHOTS.standard,
