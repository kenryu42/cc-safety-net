...
import { isReservedTransparentWrapper } from '@next/core/policy/transparent-wrappers';
import type { EffectivePolicy } from '@next/core/policy/types';
import { dangerousInTextMatch } from '@next/gate/analyzer/dangerous-text';
import {
  hasLinearDangerousText,
  hasLinearInterpreterDanger,
} from '@next/gate/analyzer/linear-danger-scanner';
import {
...
} from '@next/gate/analyzer/shell-git-env';
import {
  isStandardCommandWrapper,
  unwrapTransparentWrapper,
} from '@next/gate/analyzer/transparent-wrappers';
import { dangerousInTextMatch as shippedDangerousInTextMatch } from '@/analyzer/dangerous-text';
import {
  hasLinearDangerousText as shippedHasLinearDangerousText,
  hasLinearInterpreterDanger as shippedHasLinearInterpreterDanger,
} from '@/analyzer/linear-danger-scanner';
import {
  type ShellGitContextEnvState as ShippedShellGitContextEnvState,
  applyShellGitContextEnvSegment as shippedApplySegment,
  cloneShellGitContextEnvState as shippedCloneState,
  createShellGitContextEnvState as shippedCreateState,
  getSegmentGitContextEnvAssignments as shippedSegmentAssignments,
} from '@/analyzer/shell-git-env';
import {
  isReservedTransparentWrapper as shippedIsReserved,
...
/**
 * The four small middle-layer modules answer with the shipped ones for the corpus commands, a
 * seeded fuzz, and a table per module that walks the branches it owns. The linear scanners and
 * the raw-text matcher also charge the same scan work, so the caller's budget breaches at the
...
function snapshotState(
  state: ShellGitContextEnvState | ShippedShellGitContextEnvState,
  ...
  test('the shell Git-context tracker walks every segment to the same state', () => {
    ...
    for (const variables of GIT_ENV_ENVIRONMENTS) {
      ...
      for (const tokens of GIT_ENV_SEGMENTS) {
        ...
        const shippedAssignments = shippedSegmentAssignments(tokens, shippedState);
        expect([...(assignments ?? new Map())].sort()).toStrictEqual(
          [...(shippedAssignments ?? new Map())].sort(),
        );
        expect(assignments === undefined).toBe(shippedAssignments === undefined);

        const forked = cloneShellGitContextEnvState(state);
        applyShellGitContextEnvSegment(tokens, forked);
        const shippedForked = shippedCloneState(shippedState);
        shippedApplySegment(tokens, shippedForked);
        expect(snapshotState(forked)).toStrictEqual(snapshotState(shippedForked));
