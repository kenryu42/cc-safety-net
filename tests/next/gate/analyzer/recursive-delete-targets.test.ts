...
} from '@next/gate/analyzer/recursive-delete-targets';
import {
  type RecursiveDeleteTargetOptions as ShippedOptions,
  classifyRecursiveDeleteTarget as shippedClassify,
  createRecursiveDeleteTargetContext as shippedCreateContext,
  isDangerousRootOrHomeTarget as shippedIsDangerous,
  isTrustedTempDescendantTarget as shippedIsTrustedTempDescendant,
  deleteTargetWordFacts as shippedWordFacts,
 ...
 * is compared against the shipped classifier over the same targets, options and filesystem.
 */
...
beforeAll(() => {
  ...
  gitMetadata = resolveProtectedGitMetadata([worktrees.mainWorktree]);
});
...
function contextPair(row: ContextCase) {
  const paired = environments(row.env);
  const options = resolvedOptions(row);
  return {
    next: createRecursiveDeleteTargetContext({ ...options, environment: paired.next }),
    shipped: shippedCreateContext({
      ...options,
...
/** The context fields both implementations expose; the budget behind them is private. */
  ...
  test('every classification kind is reached by the table', () => {
    const kinds = new Set(
      contextCases().flatMap((row) => {
        const context = contextPair(row).next;
...
function words(source: string, parse: typeof parseCommand): readonly CommandWord[] {
  const node = parse(source, 'posix').nodes[0];
  return node?.kind === 'command' ? node.words : [];
}

/** Asserts both implementations derive the same facts for every word of one source. */
function expectSameWordFacts(
  source: string,
  nextWords: readonly CommandWord[],
  shippedWords: readonly CommandWord[],
) {
  expect(nextWords.length, source).toBe(shippedWords.length);
...
describe('delete target word facts', () => {
  ...
  test('the table covers expansion, both limits and the quoted $TMPDIR form', () => {
    const facts = (source: string) => {
      const word = words(source, parseCommand).at(-1);
  ...
  test('derives the same facts for every word of the corpus and the seeded fuzz', () => {
    for (const source of [
      ...
      expectSameWordFacts(
        ...
        shippedProjectCommandViews(shippedParseCommand(source, 'posix')).flatMap(
          (view) => view.words,
        ),
      );
    }
