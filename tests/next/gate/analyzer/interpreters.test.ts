...
} from '@next/gate/analyzer/interpreters';
import {
  REASON_INTERPRETER_BLOCKED as SHIPPED_REASON_BLOCKED,
  REASON_INTERPRETER_DANGEROUS as SHIPPED_REASON_DANGEROUS,
  containsDangerousCode as shippedContainsDangerousCode,
  extractInterpreterCodeArg as shippedExtractCodeArg,
  extractInterpreterExecutableSources as shippedExtractSources,
  isInterpreterDisplayOnly as shippedIsDisplayOnly,
  isInterpreterCommand as shippedIsInterpreterCommand,
...
/**
 * The interpreter argv scanner and its code detector answer with the shipped module for the
 * corpus commands, a seeded fuzz, and a table that walks each `-c`/`-e` form the four
 * interpreters accept, including the encoded payloads a one-liner hides a command in. The two
 * reasons are the strings the paranoid-interpreter and dangerous-code denials render, so they
 * are compared as values, not re-typed.
 */
