...
import {
  ...
  REASON_XARGS_RM,
  REASON_XARGS_SHELL,
} from '@next/gate/analyzer/xargs';
import { textCommandWords as shippedWords } from '@/analyzer/command-words';
import { createDerivedCommandWorkBudget as shippedDerivedBudget } from '@/analyzer/derived-command-budget';
import {
  REASON_XARGS_RM as SHIPPED_RM_REASON,
  REASON_XARGS_SHELL as SHIPPED_SHELL_REASON,
  analyzeXargs as shippedAnalyzeXargs,
  extractXargsChildCommandWithInfo as shippedExtractChild,
} from '@/analyzer/xargs';
 ...
 * spell. Both implementations answer over the same option shapes, child heads and policy states,
 * and both are asked which nested sources they hand back to the caller.
 *
 * The paths here are lexical, not a fixture tree: every verdict `xargs` reaches is decided by the
 * option scan and the child dispatch, and the canonicalization both sides share is pinned by the
 * path differentials under `tests/next/core/`.
 */
...
/** One token list through both analyzers, each with its own budget and nested-source log. */
function runBothXargs(tokens: readonly string[], setting: XargsSetting) {
  ...
  const recorder = (log: string[]) => (source: string) => {
    log.push(source);
  ...
  return {
    ...
    shipped: {
      asked: shippedLog,
      match: describeOutcome(() =>
        shippedAnalyzeXargs(shippedWords(tokens), {
          ...shared,
          analyzeNested: recorder(shippedLog),
...
describe('xargs analysis', () => {
  ...
  test('the shapes reach the dynamic-source, dynamic-rm and custom-rule verdicts', () => {
    ...
    for (const tokens of EVERY_SHAPE) {
      for (const setting of SETTINGS) {
        const verdict = runBothXargs(tokens, setting).next.match;
  ...
  test('a reader child is allowed where a deleting child is not', () => {
    ...
    const appended = runBothXargs(['xargs', 'rm', '-rf'], { label: 'defaults' }).next.match;
    ...
    expect(runBothXargs(['xargs', 'cat'], { label: 'defaults' }).next.match).toStrictEqual({
      ok: true,
      value: null,
    });
  });
  ...
  test('a disabled rule drops only the filterable verdict', () => {
    ...
    const on = runBothXargs(dynamicShell, { label: 'defaults' }).next.match;
