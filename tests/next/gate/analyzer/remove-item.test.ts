...
import type { ShellKind } from '@next/core/shell/model';
import { parseCommand } from '@next/core/shell/parse';
import { projectCommandViews } from '@next/core/shell/traversal';
import { analyzePowerShellCommandViewMatch } from '@next/gate/analyzer/powershell/remove-item';
import { createRecursiveDeleteTargetContext } from '@next/gate/analyzer/recursive-delete-targets';
import { analyzePowerShellCommandViewMatch as shippedRemoveItem } from '@/analyzer/powershell/remove-item';
import {
  type RecursiveDeleteTargetOptions as ShippedDeleteOptions,
  createRecursiveDeleteTargetContext as shippedDeleteContext,
 ...
 * spelling is parsed in all three dialects and compared against the shipped analyzer.
 */
...
/** Every command view the dialect yields, paired index by index between the two parsers. */
function viewPairs(source: string, dialect: ShellKind) {
  const nextViews = projectCommandViews(parseCommand(source, dialect));
  const shippedViews = shippedCommandViews(shippedParseCommand(source, dialect));
  return nextViews.flatMap((view, index) => {
    const shipped = shippedViews[index];
    return shipped ? [{ next: view, shipped }] : [];
  });
}
...
describe('powershell Remove-Item', () => {
  ...
  test('the table reaches every Remove-Item rule', () => {
    const reported = new Set(
      removeItemCases().flatMap((row) => {
        ...
        const options = { ...optionsFor(row), environment: paired.next };
        return REMOVE_ITEM_COMMANDS.flatMap((source) =>
          viewPairs(source, 'powershell').flatMap((pair) =>
            [false, true].flatMap((hasPipelineInput) => {
              const match = analyzePowerShellCommandViewMatch(pair.next, hasPipelineInput, options);
  ...
  test('a caller-supplied delete-target context overrides the one built from the options', () => {
    ...
    for (const pair of viewPairs('Remove-Item -Recurse -Force build', 'powershell')) {
      ...
      expect(next).toStrictEqual(
        shippedRemoveItem(
          pair.shipped,
          false,
          { ...options, environment: paired.shipped },
          shippedDeleteContext({
            ...anchoredAtHome,
            environment: paired.shipped,
          } as ShippedDeleteOptions),
        ),
      );
      ...
      expect(
        analyzePowerShellCommandViewMatch(pair.next, false, {
          ...options,
          environment: paired.next,
        }),
      ).toBeNull();
  ...
  test('-WhatIf disarms the command and an unabbreviated alias is still recognized', () => {
    ...
    const analyze = (source: string, piped = false) =>
      viewPairs(source, 'powershell').map(
        (pair) => analyzePowerShellCommandViewMatch(pair.next, piped, options)?.id ?? null,
      );
