...
import { runExplain } from '@next/cli/explain/run';
import { AnalysisLimit } from '@next/core/budget';
import { explainCommand, getConfigSource } from '@next/gate/explain';
import { StructuralShellSyntaxLimitError } from '@next/gate/guards/semantic-facts';
import { GuardEvaluationError } from '@next/gate/pipeline';
import {
  explainCommand as shippedExplainCommand,
  getConfigSource as shippedGetConfigSource,
} from '@/engine/explain';
...
/**
 * The projection itself, field by field, where the rendered rows only compare the bytes that
 * survive formatting. `src` reads its home and modes from the process, the port reads them from
 * an Environment, so each row runs the shipped function inside `withProcessEnv` and the ported
 * one against the same values as data — over one fixture, so a path in the result is the same
 * path on both sides.
 */
...
/** The two results as plain data: a frozen snapshot and a `Map`-free copy compare cleanly. */
...
function compareSides(
  ...
  const shipped = withProcessEnv(side.values, () =>
    withStdoutTTY(false, () => asData(shippedExplainCommand(command, withCwd))),
  );
