...
import {
  createCommandTraceContext as createShippedContext,
  createCommandTraceRecorder as createShippedRecorder,
} from '@/engine/command-trace';

/**
 * The ported recorder must retain, bound and redact exactly what the shipped one does, so every
 * step kind, a secret-bearing payload and each terminal shape go through both implementations.
 */

const PORTED = { recorder: createPortedRecorder, context: createPortedContext };
const SHIPPED = { recorder: createShippedRecorder, context: createShippedContext };
...
describe('ported command trace recorder', () => {
  ...
  test('freezes the trace and its events as shipped', () => {
    ...
    const shipped = recordAll(
      createShippedRecorder,
      {},
