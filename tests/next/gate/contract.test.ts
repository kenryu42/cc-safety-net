...
import { evaluateGuard, type GuardDependencies, type GuardEvaluation } from '@next/gate/pipeline';
import {
  type BehavioralContractCase,
  behavioralContractCases,
} from '../../analyzer/behavioral-contract-cases';
import {
  type PipelineContractCase,
  pipelineContractCases,
} from '../../engine/pipeline-contract-cases';
import { policySnapshot } from '../../helpers/policy';
import { writeTree } from '../helpers/fixture-tree';

/**
 * The behavioral contract through the ported gate, entered the way every host enters it, so the
 * protection stages and secret matching sit in front of command analysis for every row.
 * `tests/engine/behavioral-contract-pipeline.test.ts` pins the same rows against the shipped
 * pipeline; this file pins the port, with the process state injected as an Environment rather
 * than mutated on `process.env`.
 */
