...
import {
  commandSignature as shippedCommandSignature,
  formatRelativeTime as shippedFormatRelativeTime,
} from '@/engine/audit-display';
...
/**
 * Distances both implementations answer identically no matter which side of a millisecond they
 * read the clock on: each sits far from the boundary of the unit it prints.
 */
...
describe('audit display parity', () => {
  for (const signature of SIGNATURES) {
    test(`commandSignature ${signature.name}`, () => {
      const key = commandSignature(signature.source);
      expect(key).toBe(shippedCommandSignature(signature.source));
      expect(key).toBe(signature.key);
    });
  }

  for (const distance of DISTANCES) {
    test(`formatRelativeTime reads ${distance.name} the same way`, () => {
      const value = new Date(Date.now() - distance.ago);
      const text = formatRelativeTime(value);
      expect(text).toBe(shippedFormatRelativeTime(value));
      expect(text).toBe(distance.text);
    });
  }

  test('formatRelativeTime has nothing to say about an unparseable timestamp', () => {
    expect(formatRelativeTime('not a timestamp')).toBe('');
    expect(shippedFormatRelativeTime('not a timestamp')).toBe('');
  });
});
