import type { CommandProgram, ShellKind } from '@next/core/shell/model';
import { parseCommand as parseWithNext } from '@next/core/shell/parse';
import { parseCommand as parseWithSrc } from '@/parser/command';
...
/**
 * Shared inputs for the differential tests under `tests/next/`: the corpus commands, a fixed
 * table of parser-shaped commands, and a seeded fuzz over a shell-like alphabet. Every test file
 * feeds the same inputs to the `src/` module and the `next/` module and asserts equal output.
 */
...
export type ProgramPair = {
  ...
  readonly src: CommandProgram;
  readonly next: CommandProgram;
};

/** Every source parsed as every dialect by both implementations. */
export function differentialProgramPairs(): readonly ProgramPair[] {
  return differentialSources().flatMap((source) =>
    SHELL_DIALECTS.map((dialect) => ({
      source,
      dialect,
      src: parseWithSrc(source, dialect),
      next: parseWithNext(source, dialect),
    })),
  );
}
