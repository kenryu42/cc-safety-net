...
import { runGrokBuildHook as portedGrokBuildHook } from '@next/hosts/grok-build/hook';
import { runHermesAgentHook as portedHermesAgentHook } from '@next/hosts/hermes-agent/hook';
...
import { runGrokBuildHook as shippedGrokBuildHook } from '@/integrations/grok-build/hook';
import { runHermesAgentHook as shippedHermesAgentHook } from '@/integrations/hermes-agent/hook';
import { runKimiCodeHook as shippedKimiCodeHook } from '@/integrations/kimi-code/hook';
import { withEnv } from '../../helpers';

/**
 * One table of hosts and payloads, shared by the in-process adapter differential and the
 * process-level differential through the two bins: the same stdin bytes and the same environment
 * must produce the same document and the same audit line whichever implementation reads them.
 * Every payload is a literal document in the host's own protocol shape, so a row that stops
 * matching the host is a broken row rather than a silently rewritten one.
 */
...
/** The `pathEnvironmentExpansion` breach pinned in tests/next/gate/failure-injection.test.ts: both
 *  implementations throw inside the pipeline on it, so it drives the fail-mode rows. */
...
const HOST_SPECS: readonly HostSpec[] = [
  ...
  },
  {
    id: 'antigravity-cli',
    flag: '--agy-cli',
    shipped: shippedAntigravityCliHook,
  ...
  },
  {
    id: 'hermes-agent',
    flag: '--hermes-agent',
    shipped: shippedHermesAgentHook,
