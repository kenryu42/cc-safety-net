 ...
 * overruns its cap or never ends, a response held open). Every row points both implementations at
 * the same origin through `resolveUrl`, so no test needs the network and both sides answer to the
 * same bytes.
 */
