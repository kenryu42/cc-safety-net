...
import { createPolicyGuiServer as createPortedServer } from '@next/gui/index';
import { createPolicyGuiServer as createShippedServer } from '@/gui';
...
// Importing the ported server runs the frontend bundle at module load; see the helper.
await repairBundlerDirectoryCache();

/**
 * One seed, two loopback servers: the shipped one reads its home off `process.env` and the ported
 * one off an `Environment` built from the same values. Both answer the same request sequence, so a
 * row proves the two implementations sent the same bytes and left the same files behind rather than
 * that they agree on paper.
 */
  ...
  return {
    response: gate.then(async () => {
      ...
      return {
        ...
        body: observeBody(contentType, reply.slice(split + 4), token),
      };
    }),
  };
}
...
/** Both sides spelled the same way: each side's own root, and the path it resolves to, are `<root>`. */
...
export async function runGuiRow(row: {
  seed: TreeSpec;
  options?: (side: GuiSide, label: 'shipped' | 'ported') => GuiHookOptions;
  requests: readonly GuiRequest[];
}) {
  const shippedSide = seedSide('gui-shipped-', row.seed);
  const portedSide = seedSide('gui-ported-', row.seed);

  const shippedResponses = await withProcessEnv(shippedSide.values, async () => {
    const server = await createShippedServer({
      cwd: shippedSide.project,
      ...row.options?.(shippedSide, 'shipped'),
    });
    return drive(server, row.requests).finally(server.close);
  });
  const portedServer = await createPortedServer(
    environmentFor(portedSide.home, portedSide.values),
    { cwd: portedSide.project, ...row.options?.(portedSide, 'ported') },
  );
