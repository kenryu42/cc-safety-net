...
import type { GuardStage } from '@/engine/guard';
import type { BlockIntent } from '@/ir/decision';
import type { ToolRoute } from '@/ir/invocation';
import type { PolicySafetyLevel } from '@/ir/policy';
 ...
 * fail-closed input handling. `tests/analyzer/behavioral-contract-cases.ts`
 * owns the destructive-command rows; both corpora run through `evaluateGuard`
 * in `behavioral-contract-pipeline.test.ts`.
 */
export interface PipelineContractCase {
  ...
  /**
   * The shipped pipeline is known to decide this row wrongly. The row runs as
   * `test.failing` so the gap stays documented and the greenfield gate must
   * flip it; remove the marker in the change that closes the gap.
   */
  knownGap?: string;
}
...
export function pipelineContractCases(paths: {
  ...
  return [
    ...
    },
    {
      ...
      expected: secretBlock('secret.home.ssh'),
      knownGap:
        'an agent debugging a Git remote runs `cd ~` and then reads `.ssh/config`; secret protection walks segments without cd tracking, unlike the protected-path scanner, so the read is allowed',
    },
    {
